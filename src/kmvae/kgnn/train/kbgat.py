...
from ..models import (KBGraphAttentionNetwork,
...
CONFIG = {
    ...
    'kbgat_path': 'models/kbgat.pt',
    'kbgat_embeddings_path': 'models/kbgat_embeddings.pt',
...
def tune_hyperparameters(config):
    ...
    study = optuna.create_study(study_name='kbgat', direction='maximize')
...
def train(
    ...
    parameters = kbgat.parameter_dicts()
    optimizer = optim.AdamW(
        parameters, lr=learning_rate, weight_decay=weight_decay)

    writer = SummaryWriter(
        comment='LR{:.3g}_WD{:.3g}_NR{}'.format(
            learning_rate,
            weight_decay,
            negative_ratio))
            ...
            with torch.no_grad():
                ...
                if mean_reciprocal_rank > best_mean_reciprocal_rank:
                    ...
                    save_models(
                        kbgat,
                        triple_train_dataset.num_entities,
                        triple_train_dataset.num_relations,
                        entity_ids_val,
                        relation_ids_val,
                        triple_list_val,
                        sparse_triple_adjacency_list_indices_val,
...
def save_models(
    ...
    kbgat_embeddings = KnowledgeGraphEmbeddings(
        num_entities,
        num_relations,
        embedding_size,
        pretrained_entity_embeddings=entity_embeddings,
        pretrained_relation_embeddings=relation_embeddings)
    torch.save(
        kbgat_embeddings.state_dict(), kbgat_embeddings_path)
