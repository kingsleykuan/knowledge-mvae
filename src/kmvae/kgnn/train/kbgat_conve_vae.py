...
# NOTE: During training smaller batch size is used so context node
# captures more information from smaller graphs. Gradient accumulation
# is used to mimic batched graph encoding.
...
CONFIG = {
    ...
    'kbgat_encoder_path': 'models/kbgat_encoder',
    'conve_decoder_path': 'models/conve_decoder',
    ...
    'beta_start': 0.0,
    'beta_stop': 1.0,
    'beta_cyclical_total_steps': 200000,
    'beta_cycles': 50,
...
def tune_hyperparameters(config):
    ...
    study = optuna.create_study(
        study_name='kbgat_conve_vae', direction='maximize')
...
def train(
    ...
    kbgat_conve_vae = kbgat_conve_vae.to(device)

    parameters = kbgat_conve_vae.parameter_dicts()
...
def test(
    ...
    kbgat_conve_vae = load_models(
        triple_test_dataset, kbgat_encoder_path, conve_decoder_path)
    kbgat_conve_vae = kbgat_conve_vae.to(device)
...
def create_models(
    ...
    if pretrained_conve_path:
        conve_state_dict = torch.load(pretrained_conve_path)
    else:
    ...
    kbgat_conve_vae = VariationalAutoencoder(
        kbgat,
        conve,
        kl_free_bits=kl_free_bits,
        beta_schedule=beta_schedule,
...
def load_models(
        ...
        beta_cycle_ratio_zero=0.5,
        beta_cycle_ratio_increase=0.25):
    kbgat = KBGraphAttentionNetworkEncoder.from_pretrained(kbgat_encoder_path)
    conve = ConvEDecoder.from_pretrained(conve_decoder_path)
...
def train_loop(
        ...
        steps_per_log,
        gradient_accumulation_steps):
    kbgat_conve_vae = kbgat_conve_vae.train()
    ...
    for step, data in enumerate(data_loader):
        ...
        loss_normalized = loss / gradient_accumulation_steps
...
def train_step(
    ...
    triple_labels = torch.cat((
        torch.ones(len(triples), device=device),
        torch.zeros(len(corrupted_triples), device=device)), 0)
    ...
    outputs = kbgat_conve_vae(encoder_inputs, decoder_inputs, calc_loss=True)
...
def eval_loop(
    ...
    kbgat_conve_vae = kbgat_conve_vae.eval()
...
def eval_step(
    ...
    outputs = kbgat_conve_vae(encoder_inputs, decoder_inputs)
...
def save_models(kbgat_conve_vae, kbgat_encoder_path, conve_decoder_path,):
    kbgat_conve_vae = kbgat_conve_vae.eval()
    kbgat_conve_vae.encoder.save_pretrained(kbgat_encoder_path)
    kbgat_conve_vae.decoder.save_pretrained(conve_decoder_path)
