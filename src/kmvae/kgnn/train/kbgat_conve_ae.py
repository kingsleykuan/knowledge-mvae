...
CONFIG = {
    ...
    'pretrained_embeddings_path': 'models_200/transe_embeddings.pt',
    'pretrained_kbgat_path': None,
    'pretrained_conve_path': None,
    'kbgat_path': 'models_200/kbgat_ae_new.pt',
    'conve_path': 'models_200/conve_ae_new.pt',
...
def tune_hyperparameters(config):
    ...
    study = optuna.create_study(
        study_name='kbgat_conve_ae', direction='maximize')
...
def train(
    ...
    parameters = kbgat.parameter_dicts() + conve.parameter_dicts()
        ...
        if epoch % epochs_per_val == 0:
            with torch.no_grad():
                ...
                if mean_reciprocal_rank > best_mean_reciprocal_rank:
                    ...
                    best_epoch = epoch
                    save_models(kbgat, conve, kbgat_path, conve_path)
...
def save_models(kbgat, conve, kbgat_path, conve_path):
    kbgat = kbgat.eval()
    conve = conve.eval()
