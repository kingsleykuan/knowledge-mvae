"""ConvKB Knowledge Graph Embeddings."""
import optuna
import torch
...
# Mean Rank: 233.2, Mean Reciprocal Rank: 0.29751
# Hits@1: 0.20966, Hits@3: 0.3284, Hits@10: 0.47288
CONFIG = {
    ...
    'pretrained_conv_kb_path': None,
    'convkb_embeddings_path': 'models/convkb_embeddings.pt',
    'convkb_path': 'models/convkb.pt',
...
def tune_hyperparameters(config):
    ...
    study = optuna.create_study(study_name='convkb', direction='maximize')
...
def train(
        ...
        pretrained_conv_kb_path,
        convkb_embeddings_path,
        convkb_path,
        freeze_embeddings,
        num_epochs,
        epochs_per_val,
    ...
    knowledge_graph_embeddings, convkb = load_models(
        ...
        pretrained_conv_kb_path=pretrained_conv_kb_path)
    knowledge_graph_embeddings = knowledge_graph_embeddings.to(device)
    convkb = convkb.to(device)

    parameters = [
        {'params': knowledge_graph_embeddings.parameters(), 'weight_decay': 0},
        {'params': convkb.parameters()}
    ]
...
def test(
    ...
    knowledge_graph_embeddings, convkb = load_models(
        ...
        embedding_size,
        num_filters,
        pretrained_embeddings_path=convkb_embeddings_path,
        pretrained_conv_kb_path=convkb_path)
    knowledge_graph_embeddings = knowledge_graph_embeddings.to(device)
    convkb = convkb.to(device)
...
def load_models(
    ...
    if pretrained_conv_kb_path:
        convkb.load_state_dict(torch.load(pretrained_conv_kb_path))

    return knowledge_graph_embeddings, convkb
...
def save_models(
    ...
    knowledge_graph_embeddings = knowledge_graph_embeddings.eval()
    convkb = convkb.eval()
    torch.save(knowledge_graph_embeddings.state_dict(), convkb_embeddings_path)
